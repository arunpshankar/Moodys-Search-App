...
def search_data_store(search_query: str, batch_id: str) -> Optional[discoveryengine.SearchResponse]:
    """
...
if __name__ == "__main__":
    search_query = "Musashino Bank Annual Report 2021 Japan"
    row_info = find_entity_url_by_key('Musashino Bank', 'Japan')
    batch_id = row_info['batch_id']

    try:
        results = search_data_store(search_query, batch_id)
        if results:
